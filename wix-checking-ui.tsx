...
export default function WixCheckingUI() {
...
function NavItem({ label, badge, expandable, expanded, onToggle, active }) {
  return (
    <div 
...
function SubNavItem({ label, active, badge }) {
  return (
    <div className={`px-6 py-2 flex items-center justify-between cursor-pointer hover:bg-slate-800 ${active ? 'bg-slate-800 border-l-2 border-blue-500' : ''}`}>
      <span className="text-sm">{label}</span>
      {badge && (
...
function Transaction({ icon, title, subtitle, amount }) {
